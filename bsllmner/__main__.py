...
def main():
    ...
    parser.add_argument('input_json_filename', help='Input JSON file')
    ...
    if mode == "extract":
        bsner = ollama_chat.BsNer(input_json_filename, model, prompt_filename, prompt_indices, host_url)
        bsner.ner(verbose, test)
    elif mode == "review":
        bsreview = ollama_chat.BsReview(input_json_filename, model, prompt_filename, prompt_indices, host_url, metasra_tsv, llmner_tsv)
